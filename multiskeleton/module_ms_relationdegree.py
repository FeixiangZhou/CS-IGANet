...
class S1_to_S2(nn.Module):
    ...
    def __init__(self, t, n_j1, n_j2, n_p1, n_p2, t_kernel, t_stride, t_padding, weight):
        ...
        self.weight = nn.Parameter(torch.Tensor(1, 256))
        self.weight2 = nn.Parameter(torch.Tensor(1, 256))

        self.weight_t12t2 = nn.Parameter(torch.Tensor(1, 256))
        self.weight_t22t1 = nn.Parameter(torch.Tensor(1, 256))

        self.weight_t12t2_s2 = nn.Parameter(torch.Tensor(1, 256))
        self.weight_t22t1_s2 = nn.Parameter(torch.Tensor(1, 256))
    ...
    def forward(self, x_s1, x_s2, relrec_s1, relsend_s1, relrec_s2, relsend_s2, relrec_speed, relsend_speed, A_joint2part, A_keleton2skeleton_s1, A_keleton2skeleton_s2):                                                           # x_s1: [64, 32, 49, 26]
        ...
        # print("S1_to_S2_x_s1_att-----", x_s1_att.shape, x_s2_att.shape)
        # print("S1_to_S2_x_s1_att-----", x_s1_att[0,:,0])
        # print("S1_to_S2_x_s2_att-----", x_s2_att[0, :, 0])

        # Att = self.softmax(torch.matmul(x_s1_att, x_s2_att.permute(0,2,1)).permute(0,2,1))                                 # [64, 3, 8]
        ...
        x_s1_att_t1t2_cat = torch.cat([x_s1_att_t1.repeat(1, 1, V).view(N2, V* V, -1), x_s1_att_t2.repeat(1, V, 1)], dim=2).view(N2, V, -1, 2 * 128)
        x_s1_att_t1t2_cat = x_s1_att_t1t2_cat.permute(0, 1, 3, 2)
        Att_s1_t12t2 = torch.matmul(self.weight_t12t2, x_s1_att_t1t2_cat).squeeze(2) #(N, 7,7)
        Att_s1_t12t2 = self.relu(Att_s1_t12t2)


        Att_s1_t12t2_noself = self.softmax(Att_s1_t12t2)
        ...
        #re calculate relation  degree
        x_s1_att_t2t1_cat = torch.cat([x_s1_att_t2.repeat(1, 1, V).view(N2, V* V, -1), x_s1_att_t1.repeat(1, V, 1)], dim=2).view(N2, V, -1, 2 * 128)
        x_s1_att_t2t1_cat = x_s1_att_t2t1_cat.permute(0, 1, 3, 2)
        Att_s1_t22t1 = torch.matmul(self.weight_t22t1, x_s1_att_t2t1_cat).squeeze(2) #(N, 7,7)
        ...
        # print(x_s1_t2t1_afterinter.shape)
        ...
        x_s2_att_t1t2_cat = torch.cat([x_s2_att_t1.repeat(1, 1, W).view(N2, W* W, -1), x_s2_att_t2.repeat(1, W, 1)], dim=2).view(N2, W, -1, 2 * 128)
        x_s2_att_t1t2_cat = x_s2_att_t1t2_cat.permute(0, 1, 3, 2)
        Att_s2_t12t2 = torch.matmul(self.weight_t12t2_s2, x_s2_att_t1t2_cat).squeeze(2) #(N, 3,3)
        ...
        x_s2_att_t2t1_cat = torch.cat([x_s2_att_t2.repeat(1, 1, W).view(N2, W* W, -1), x_s2_att_t1.repeat(1, W, 1)], dim=2).view(N2, W, -1, 2 * 128)
        x_s2_att_t2t1_cat = x_s2_att_t2t1_cat.permute(0, 1, 3, 2)
        Att_s2_t22t1 = torch.matmul(self.weight_t22t1_s2, x_s2_att_t2t1_cat).squeeze(2) #(N, 7,7)
        ...
        Att = torch.cat([x_s2_att.repeat(1, 1, V).view(N, V * W, -1), x_s1_att.repeat(1, W, 1)], dim=2).view(N, W, -1, 2 * 128)
        Att = Att.permute(0, 1, 3, 2)
        Att = torch.matmul(self.weight, Att).squeeze(2)  # (N, 3,7)
        Att = self.relu(Att)
       ...
       #cross S1 t2 to  S2 t1
        ...
        Att_s1t2tos2t1 = torch.cat([x_s2_att.repeat(1, 1, V).view(N, V * W, -1), x_s1_att_t2t1.repeat(1, W, 1)], dim=2).view(N, W, -1, 2 * 128)
        Att_s1t2tos2t1 = Att_s1t2tos2t1.permute(0, 1, 3, 2)
        Att_s1t2tos2t1 = torch.matmul(self.weight2, Att_s1t2tos2t1).squeeze(2)  # (N, 3,7)
        Att_s1t2tos2t1 = self.relu(Att_s1t2tos2t1)
...
class S2_to_S1(nn.Module):
    ...
    def forward(self, x_s2, x_s1, relrec_s2, relsend_s2, relrec_s1, relsend_s1, relrec_speed, relsend_speed, A_joint2part, A_keleton2skeleton_s1,A_keleton2skeleton_s2):
        ...
        x_s2_att_t1t2_cat = torch.cat([x_s2_att_t1.repeat(1, 1, W).view(N2, W* W, -1), x_s2_att_t2.repeat(1, W, 1)], dim=2).view(N2, W, -1, 2 * 128)
        x_s2_att_t1t2_cat = x_s2_att_t1t2_cat.permute(0, 1, 3, 2)
        Att_s2_t12t2 = torch.matmul(self.weight_t12t2_s2, x_s2_att_t1t2_cat).squeeze(2) #(N, 7,7)
        ...
        Att = torch.cat([x_s1_att.repeat(1, 1, W).view(N, V * W, -1), x_s2_att.repeat(1, V, 1)], dim=2).view(N, V, -1, 2 * 128)
        Att = Att.permute(0, 1, 3, 2)
        Att = torch.matmul(self.weight, Att).squeeze(2)  # (N, 7,3)
        Att = self.relu(Att)
        ...
        Att_s2t2tos1t1 = torch.cat([x_s1_att.repeat(1, 1, W).view(N, V * W, -1), x_s2_att_t2t1.repeat(1, V, 1)], dim=2).view(N, V, -1, 2 * 128)
        Att_s2t2tos1t1 = Att_s2t2tos1t1.permute(0, 1, 3, 2)
        Att_s2t2tos1t1 = torch.matmul(self.weight2, Att_s2t2tos1t1).squeeze(2)  # (N, 7,3)
        Att_s2t2tos1t1 = self.relu(Att_s2t2tos1t1)
        ...
        x_s2 = x_s2_t2t1_afterinter.permute(0, 3, 2, 1).contiguous().view(N, W,-1)  # [128, 3, 40, 64] -> [128, 3, 2560]
        x_s1_s2t2tos1t1 = torch.einsum('nwv, nvd->nwd', (Att_s2t2tos1t1, x_s2))  # [128, 7, 2560]
        # print("S1_to_S2_x_s2_glb-----", x_s2_glb.shape)
...
class interskeleton_interaction(nn.Module):   #1280 t0 120
    ...
    def __init__(self,t,  n_j1, n_j2, n_p1, n_p2, t_kernel, t_stride, t_padding, weight):
        ...
        self.weight_t12t2 = nn.Parameter(torch.Tensor(1, 1280))
        self.weight_t22t1 = nn.Parameter(torch.Tensor(1, 1280))
        self.relu = nn.ReLU(inplace=True)
    ...
    def forward(self, x_s1, relrec_s1, relsend_s1, relrec_speed, relsend_speed):                                                           # x_s1: [64, 32, 49, 26]
        ...
        # x_s1_att = self.embed_s1(x_s1, relrec_s1, relsend_s1, relrec_speed, relsend_speed)  # (128,8, 128)
        ...
        x_s1_att_t1t2_cat = torch.cat([x_s1_att_t1.repeat(1, 1, V).view(N2, V* V, -1), x_s1_att_t2.repeat(1, V, 1)], dim=2).view(N2, V, -1, 2 * 640)
        x_s1_att_t1t2_cat = x_s1_att_t1t2_cat.permute(0, 1, 3, 2)
        Att_s1_t12t2 = torch.matmul(self.weight_t12t2, x_s1_att_t1t2_cat).squeeze(2) #(N, 7,7)
...
class interskeleton_interaction_S2(nn.Module):  #120 -1280 nodes=3
    ...
    def __init__(self,t,  n_j1, n_j2, n_p1, n_p2, t_kernel, t_stride, t_padding, weight):
        ...
        # self.embed_s2 = S2AttInform(n_j1, n_j2, t_stride[1], t_kernel, t_padding, drop=0, nmp=True)
